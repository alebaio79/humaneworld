 ...
 * Enya Form Validation Controller - Survey Pages
 * Created: Winter 2021
...
function handleENValidateEvent() {
  ...
  // Deal with the captcha, if any. For now, this should affect only the signup
  // form embedded on the HSI web site. We need to deal with error message
  // styling on other forms.
  if (window.grecaptcha && state.thisPage.isHSIWebSiteSignup) {
    const captchaValid = validationHelpers.isCaptchaValid();
    if (!captchaValid) {
      validationHelpers.showCaptchaError();
    }
...
function initInstantValidation() {
  // If we're on the embedded HSUS signup page in the footer of
  // humanesociety.org, we'll find the "hsusfooter" class, which is on the body
  // tag of the EN form.
  const isHSUSFooterForm = document.querySelector(".hsusfooter");

  // This is the form tag, which is on almost every EN form page.
  const theForm = document.querySelector(".en__component--page");

  if (theForm) {
    if (isHSUSFooterForm) {
      // HSUS website footer iframe needs browser validation.
      theForm.noValidate = false;
    } else {
