...
jQuery(document).ready(function ($) {
	...
	$('input#en__field_transaction_recurrpay1').click(function () {
		...
		setTimeout(function () {
			...
			$('input[name="transaction.donationAmt.other"]').attr('placeholder', '€ Other import');
	...
	$('input[name="transaction.donationAmt.other"]').on('keyup', function () {
		...
		if (parseFloat(this.value.replace(',', '.')) >= min_amount) {
			...
			this.setCustomValidity("The minimum donation amount is 5 euros. Please enter an amount equal to or greater than 5 euros.");
			this.reportValidity();
			$(".next-btn").attr("disabled", "disabled");
			toggleFormStepTwo(true);
		}
	...
	$(document.body).on('click', 'div#FormStepSlider ul li', function () {
		...
		if (!$(this).hasClass('active')) {
			...
			if ($(this).is(':first-child')) {
				...
				$('.action-button').find('button').text('Continue');
	...
	$('#en__field_transaction_ccnumber').on('keyup', function () {
		if ($(this).val().length != 16) {
			$(this).after('<div class="en__field__error">Card number is mandatory (16 digits).</div>');
		} else {
	...
	$('#en__field_transaction_ccexpire').on('keyup', function () {
		if ($(this).val().trim().length != 2) {
			$(this).after('<div class="en__field__error">The expiration month is mandatory (2 digits).</div>');
		} else {
			$(this).parent().find('.en__field__error').remove();
		}
	});
	$('.en__field__input--splittext:not(#en__field_transaction_ccexpire)').on('keyup', function () {
		if ($(this).val().trim().length != 2) {
			$(this).after('<div class="en__field__error">Expiration year is mandatory (2 digits).</div>');
		} else {
			$(this).parent().find('.en__field__error').remove();
		}
	});
	$('#en__field_transaction_ccvv').on('keyup', function () {
		if ($(this).val().trim().length != 3) {
			$(this).after('<div class="en__field__error">CVV is mandatory (3 digits).</div>');
		} else {
			$(this).parent().find('.en__field__error').remove();
		}
	});
