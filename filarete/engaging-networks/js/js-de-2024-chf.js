...
jQuery(document).ready(function ($) {
	...
	$('input#en__field_transaction_recurrpay1').click(function () {
		...
		setTimeout(function () {
			...
			$('input[name="transaction.donationAmt.other"]').attr('placeholder', 'CHF Anderer Betrag');
	...
	$('input[name="transaction.donationAmt.other"]').on('keyup', function () {
		...
		if (parseFloat(this.value.replace(',', '.')) >= min_amount) {
			...
			this.setCustomValidity("Der Mindestbetrag für Spenden ist 5 Euro. Bitte trage eine Summe ein, die gleich oder größer als 5 Euro ist.");
			this.reportValidity();
			$(".next-btn").attr("disabled", "disabled");
		}
	...
	$(document.body).on('click', '.action-button .next-btn', function (e) {
		...
		$('.credit_card_form .en__submit button').text('Spende' + ' CHF' + selectdonate + isMonthly);
	...
	$(document.body).on('click', 'div#FormStepSlider ul li', function () {
		...
		if (!$(this).hasClass('active')) {
			...
			if ($(this).is(':first-child')) {
				...
				$('.action-button').find('button').text('Weiter');
				...
				// Stripe built-in - Add amount to button
				$('.credit_card_form .en__submit button').text('Spende' + ' CHF' + selectdonate);
	...
	$('#en__field_transaction_ccnumber').on('keyup', function () {
		if ($(this).val().length != 16) {
			$(this).after('<div class="en__field__error">Kartennummer ist obligatorisch (16 Ziffern).</div>');
		} else {
	...
	$('#en__field_transaction_ccexpire').on('keyup', function () {
		if ($(this).val().trim().length != 2) {
			$(this).after('<div class="en__field__error">Ablaufmonat ist erforderlich (2 Ziffern).</div>');
		} else {
			$(this).parent().find('.en__field__error').remove();
		}
	});
	$('.en__field__input--splittext:not(#en__field_transaction_ccexpire)').on('keyup', function () {
		if ($(this).val().trim().length != 2) {
			$(this).after('<div class="en__field__error">Ablaufjahr ist obligatorisch (2 Ziffern).</div>');
		} else {
			$(this).parent().find('.en__field__error').remove();
		}
	});
	$('#en__field_transaction_ccvv').on('keyup', function () {
		if ($(this).val().trim().length != 3) {
			$(this).after('<div class="en__field__error">CVV ist obligatorisch (3 Ziffern).</div>');
		} else {
			$(this).parent().find('.en__field__error').remove();
		}
	});
...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '#en__field_transaction_recurrpay0', function() {
        $('.en__submit.aggiungimensilebottonestile-de').removeClass('aggiungimensilebottonestile-de');
        readyToAdd = false;
    });
    ...
    $(document).on('click', '.step.two', function() {
        if (readyToAdd) {
            $('.en__submit').addClass('aggiungimensilebottonestile-de');
